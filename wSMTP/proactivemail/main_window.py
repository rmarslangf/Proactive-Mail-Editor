"""
main_window.py — Ana uygulama penceresi (MainWindow).

Sekmeli sol panel:
  Ayarlar  — Header, Footer, Renkler, Font
 İçerik  — Blok ekleme + sıralama + düzenleme

Sağ panel: Canlı HTML önizleme (QWebEngineView)

Özellikler:
  - TR / EN dil değiştirme
  - Otomatik kayıt (kapanışta + 1.5sn sonra)
  - Proje kaydet / yükle (JSON)
  - HTML dışa aktar (thread'li, arayüz donmaz)
  - HTML panoya kopyala
"""
...
from .smtp_sender import SmtpConfig
from .tabs.recipients_tab import RecipientsTab
...
class MainWindow(QMainWindow):
    ...
    def _init_state(self):
        ...
        self.smtp_password = ""
        self.smtp_from_name= ""
        self.smtp_conn_type = "starttls"   # starttls | ssl | plain
        self.smtp_delay_ms  = 300
    ...
    def _build_editor(self):
        ...
        #  SMTP Ayarları sekmesi (Ayarlar içine gömülü)
        self._build_smtp_settings(sv)
        ...
        # SEKME 3–5: SMTP SEKMELERİ 
        # Alıcılar
        self._recipients_tab = RecipientsTab(self.db)
        self._tabs.addTab(self._recipients_tab, "👥  Alıcılar")

        # Gönder
        self._send_tab = SendTab(
            db=self.db,
            get_html_fn=self._get_html,
            get_subject_fn=lambda: self.subject_edit.text().strip(),
            get_smtp_fn=self._get_smtp_config,
        )
        self._tabs.addTab(self._send_tab, "🚀  Gönder")
        ...
        self._tabs.addTab(self._logs_tab, "📋  Geçmiş")

        # Alıcı değişince gönder sekmesini güncelle
        self._recipients_tab.recipients_changed.connect(
            self._send_tab.refresh_groups)
    ...
    def _build_smtp_settings(self, parent_layout):
        """Ayarlar sekmesine SMTP / Exim yapılandırma grubu ekler."""
        from PyQt5.QtWidgets import QGroupBox, QFormLayout, QSpinBox, QCheckBox
        from .smtp_sender import MODE_GMAIL, MODE_EXIM
        SP = ("background:#0d1b2a;color:#e8e4d9;border:1px solid #0f3460;"
              "border-radius:6px;padding:5px;")

        smtp_box = QGroupBox("SMTP / EXIM AYARLARI")
        sf = QFormLayout(smtp_box)
        sf.setSpacing(8)

        #  Mod seçimi
        self._smtp_mode_cb = QComboBox()
        self._smtp_mode_cb.addItem("Gmail",                         "gmail")
        self._smtp_mode_cb.addItem("Outlook / Hotmail / Office 365","outlook")
        self._smtp_mode_cb.addItem("Özel SMTP Sunucu",              MODE_GMAIL)
        self._smtp_mode_cb.addItem("Exim (Local Relay — auth yok)", MODE_EXIM)
        self._smtp_mode_cb.currentIndexChanged.connect(self._on_smtp_mode_change)
        sf.addRow("Mod:", self._smtp_mode_cb)

        #  Bağlantı
        self._smtp_host_e = QLineEdit(self.smtp_host)
        self._smtp_port_e = QSpinBox()
        self._smtp_port_e.setRange(1, 65535)
        self._smtp_port_e.setValue(self.smtp_port)
        self._smtp_port_e.setStyleSheet(SP)
        sf.addRow("Sunucu (host):", self._smtp_host_e)
        sf.addRow("Port:",          self._smtp_port_e)

        #  Kimlik (Gmail)
        self._smtp_user_e = QLineEdit(self.smtp_user)
        self._smtp_user_e.setPlaceholderText("ornek@gmail.com")
        self._smtp_pass_e = QLineEdit(self.smtp_password)
        self._smtp_pass_e.setEchoMode(QLineEdit.Password)
        self._smtp_pass_e.setPlaceholderText("Uygulama şifresi (Gmail App Password)")
        self._smtp_conn_cb = QComboBox()
        self._smtp_conn_cb.addItem("STARTTLS  (port 587 — Gmail, kurumsal)",  "starttls")
        self._smtp_conn_cb.addItem("SSL / TLS  (port 465)",                   "ssl")
        self._smtp_conn_cb.addItem("Düz bağlantı  (port 25 — Local Exim)",    "plain")
        # Kayıtlı değeri yükle
        _ci = {"starttls":0,"ssl":1,"plain":2}.get(self.smtp_conn_type, 0)
        self._smtp_conn_cb.setCurrentIndex(_ci)
        self._smtp_conn_cb.currentIndexChanged.connect(self._on_conn_type_change)
        self._lbl_user = sf.addRow("Kullanıcı:", self._smtp_user_e)
        self._lbl_pass = sf.addRow("Şifre:",     self._smtp_pass_e)
        sf.addRow("Bağlantı tipi:", self._smtp_conn_cb)

        #  Gönderen kimliği
        self._smtp_from_e  = QLineEdit(self.smtp_user)
        self._smtp_from_e.setPlaceholderText("gonderen@sirket.com")
        self._smtp_name_e  = QLineEdit(self.smtp_from_name)
        self._smtp_name_e.setPlaceholderText("Şirket Adı")
        sf.addRow("Gönderen e-posta:", self._smtp_from_e)
        sf.addRow("Gönderen adı:",     self._smtp_name_e)

        #  Bounce / Return-Path
        self._smtp_bounce_e = QLineEdit()
        self._smtp_bounce_e.setPlaceholderText(
            "bounce@sirket.com  (boş bırakılırsa gönderen adresi kullanılır)")
        self._smtp_replyto_e = QLineEdit()
        self._smtp_replyto_e.setPlaceholderText(
            "destek@sirket.com  (opsiyonel)")
        sf.addRow("Bounce / Return-Path:", self._smtp_bounce_e)
        sf.addRow("Reply-To:",             self._smtp_replyto_e)

        #  Hız ayarları
        self._smtp_delay_e = QSpinBox()
        self._smtp_delay_e.setRange(0, 10000)
        self._smtp_delay_e.setValue(self.smtp_delay_ms)
        self._smtp_delay_e.setSuffix(" ms")
        self._smtp_delay_e.setStyleSheet(SP)
        self._smtp_batch_e = QSpinBox()
        self._smtp_batch_e.setRange(0, 10000)
        self._smtp_batch_e.setValue(0)
        self._smtp_batch_e.setSpecialValueText("Sınırsız")
        self._smtp_batch_e.setStyleSheet(SP)
        sf.addRow("Mail arası bekleme:",     self._smtp_delay_e)
        sf.addRow("Toplu gönderim (batch):", self._smtp_batch_e)

        #  Sağlayıcı ipucu
        self._smtp_hint_lbl = QLabel("")
        self._smtp_hint_lbl.setStyleSheet("color:#5a8a7a;font-size:11px;font-style:italic;")
        self._smtp_hint_lbl.setWordWrap(True)
        sf.addRow("", self._smtp_hint_lbl)
        ...
        # Kaydet butonu
        btn_save_smtp = QPushButton("💾  Ayarları Kaydet")
        btn_save_smtp.setObjectName("accent")
        btn_save_smtp.clicked.connect(self._save_smtp_to_db)
        btn_save_smtp.clicked.connect(
            lambda: QMessageBox.information(self, "Kaydedildi",
                "SMTP ayarları veritabanına kaydedildi."))
        sf.addRow("", btn_save_smtp)

        parent_layout.addWidget(smtp_box)

        # İlk mod güncelle
        self._on_smtp_mode_change()
        # DB'den ayarları yükle
        self._load_smtp_from_db()

    def _on_smtp_mode_change(self):
        if not hasattr(self, '_smtp_mode_cb'): return
        mode = self._smtp_mode_cb.currentData()
        is_exim = (mode == "exim")

        # Sağlayıcıya göre otomatik ayarlar
        presets = {
            "gmail":   ("smtp.gmail.com",       587, "starttls"),
            "outlook": ("smtp.office365.com",   587, "starttls"),
            "exim":    ("localhost",              25, "plain"),
        }
        if mode in presets:
            host, port, conn = presets[mode]
            self._smtp_host_e.setText(host)
            self._smtp_port_e.setValue(port)
            ci = {"starttls":0,"ssl":1,"plain":2}.get(conn, 0)
            self._smtp_conn_cb.setCurrentIndex(ci)

        # Exim: kimlik alanlarını gizle
        self._smtp_user_e.setVisible(not is_exim)
        self._smtp_pass_e.setVisible(not is_exim)
        self._smtp_conn_cb.setVisible(not is_exim)

        # Hint metni güncelle
        hints = {
            "gmail":   "Gmail → Google Hesabım > Güvenlik > Uygulama Şifresi",
            "outlook": "Outlook → Hesap Ayarları > Uygulama Şifresi (2FA açıksa)",
            "exim":    "Exim local relay — kullanıcı adı / şifre gerekmez",
        }
        if hasattr(self, "_smtp_hint_lbl"):
            self._smtp_hint_lbl.setText(hints.get(mode, ""))

    def _get_smtp_config(self) -> SmtpConfig:
        """Mevcut SMTP / Exim ayarlarını SmtpConfig nesnesi olarak döndürür."""
        if not hasattr(self, '_smtp_host_e'):
            return SmtpConfig(host=self.smtp_host, port=self.smtp_port,
                              user=self.smtp_user, password=self.smtp_password)
        return SmtpConfig(
            mode=self._smtp_mode_cb.currentData(),
            host=self._smtp_host_e.text().strip(),
            port=self._smtp_port_e.value(),
            user=self._smtp_user_e.text().strip(),
            password=self._smtp_pass_e.text(),
            from_email=self._smtp_from_e.text().strip(),
            from_name=self._smtp_name_e.text().strip(),
            bounce_addr=self._smtp_bounce_e.text().strip(),
            reply_to=self._smtp_replyto_e.text().strip(),
            use_tls=(self._smtp_conn_cb.currentData() != "plain"),
            delay_ms=self._smtp_delay_e.value(),
            batch_size=self._smtp_batch_e.value(),
        )

    def _on_conn_type_change(self):
        """Bağlantı tipi değişince portu otomatik öner."""
        if not hasattr(self, "_smtp_conn_cb"): return
        t = self._smtp_conn_cb.currentData()
        port_map = {"starttls": 587, "ssl": 465, "plain": 25}
        self._smtp_port_e.setValue(port_map.get(t, 587))
        self._save_smtp_to_db()

    def _save_smtp_to_db(self):
        """Mevcut SMTP ayarlarını veritabanına kaydet."""
        if not hasattr(self, "_smtp_host_e") or not hasattr(self, "db"): return
        self.db.save_settings({
            "smtp_mode":        self._smtp_mode_cb.currentData(),
            "smtp_host":        self._smtp_host_e.text().strip(),
            "smtp_port":        str(self._smtp_port_e.value()),
            "smtp_user":        self._smtp_user_e.text().strip(),
            "smtp_password":    self._smtp_pass_e.text(),
            "smtp_from_email":  self._smtp_from_e.text().strip(),
            "smtp_from_name":   self._smtp_name_e.text().strip(),
            "smtp_bounce":      self._smtp_bounce_e.text().strip(),
            "smtp_reply_to":    self._smtp_replyto_e.text().strip(),
            "smtp_conn_type":   self._smtp_conn_cb.currentData(),
            "smtp_delay_ms":    str(self._smtp_delay_e.value()),
            "smtp_batch_size":  str(self._smtp_batch_e.value()),
        })

    def _load_smtp_from_db(self):
        """Veritabanından SMTP ayarlarını yükle."""
        if not hasattr(self, "_smtp_host_e") or not hasattr(self, "db"): return
        s = self.db.get_all_settings()
        if not s: return
        if "smtp_mode" in s:
            idx = {"gmail":0,"outlook":1,"smtp":2,"exim":3}.get(s["smtp_mode"], 0)
            self._smtp_mode_cb.setCurrentIndex(idx)
        if "smtp_host"       in s: self._smtp_host_e.setText(s["smtp_host"])
        if "smtp_port"       in s: self._smtp_port_e.setValue(int(s["smtp_port"]))
        if "smtp_user"       in s: self._smtp_user_e.setText(s["smtp_user"])
        if "smtp_password"   in s: self._smtp_pass_e.setText(s["smtp_password"])
        if "smtp_from_email" in s: self._smtp_from_e.setText(s["smtp_from_email"])
        if "smtp_from_name"  in s: self._smtp_name_e.setText(s["smtp_from_name"])
        if "smtp_bounce"     in s: self._smtp_bounce_e.setText(s["smtp_bounce"])
        if "smtp_reply_to"   in s: self._smtp_replyto_e.setText(s["smtp_reply_to"])
        if "smtp_conn_type"  in s:
            idx = {"starttls":0,"ssl":1,"plain":2}.get(s["smtp_conn_type"], 0)
            self._smtp_conn_cb.setCurrentIndex(idx)
        if "smtp_delay_ms"   in s: self._smtp_delay_e.setValue(int(s["smtp_delay_ms"]))
        if "smtp_batch_size" in s: self._smtp_batch_e.setValue(int(s["smtp_batch_size"]))
        self._on_smtp_mode_change()
    ...
    def _test_smtp(self):
        ...
        if ok:
            ...
            QMessageBox.critical(self, "SMTP Test", msg)
    ...
    def _get_html(self):
        try:
            return build_html(
                subject            = self.subject_edit.text().strip() or "Campaign",
    ...
    def _project_data(self):
        return {
            ...
            "global_font_size":   self.global_font_size,
            "global_font_family": self.global_font_family,
            "smtp_host":      getattr(self, "_smtp_host_e", None) and self._smtp_host_e.text() or self.smtp_host,
            "smtp_port":      getattr(self, "_smtp_port_e", None) and self._smtp_port_e.value() or self.smtp_port,
            "smtp_user":      getattr(self, "_smtp_user_e", None) and self._smtp_user_e.text() or self.smtp_user,
            "smtp_from_name": getattr(self, "_smtp_name_e", None) and self._smtp_name_e.text() or self.smtp_from_name,
            "smtp_conn_type": getattr(self, "_smtp_conn_cb", None) and self._smtp_conn_cb.currentData() or self.smtp_conn_type,
            "blocks":            [b.get_data() for b in self.blocks],
        }
    ...
    def _apply_project_data(self, data):
        ...
        # SMTP
        if hasattr(self, "_smtp_host_e"):
            self._smtp_host_e.setText(data.get("smtp_host", self.smtp_host))
            self._smtp_port_e.setValue(data.get("smtp_port", self.smtp_port))
            self._smtp_user_e.setText(data.get("smtp_user", self.smtp_user))
            self._smtp_name_e.setText(data.get("smtp_from_name", self.smtp_from_name))
            ct = data.get("smtp_conn_type", self.smtp_conn_type)
            ci = {"starttls":0,"ssl":1,"plain":2}.get(ct, 0)
            self._smtp_conn_cb.setCurrentIndex(ci)
