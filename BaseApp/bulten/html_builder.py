"""
html_builder.py — HTML şablon üreticisi.
"""
    ...
    for blk in blocks:
        ...
        if t == "heading":
            ...
            _html = (f'<p style="font-family:{_ff};color:{_tc};font-size:{_fs}px;'
                     f'line-height:1.9;margin:{blk.get("para_margin_top",12)}px 0 {blk.get("para_margin_bottom",12)}px;">{_content_safe}</p>')
            body_parts += _wrap_url(_html, blk.get("block_url",""))
        elif t == "bullet_list":
            _ff = blk.get("font_family", global_font_family)
            _fs = blk.get("font_size", global_font_size)
